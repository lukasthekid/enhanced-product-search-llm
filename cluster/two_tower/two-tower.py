...
DECAY_STEPS = 10000
PATIENCE = 5
SIMILARITY_THRESHOLD = 0.8  # Threshold to filter out low similarity pairs
#MODEL_CHECKPOINT_PATH = 'best_model_tow_tower.keras'
ALL_CANDIDATES = True

MODEL_NAME = f'{SIMILARITY_THRESHOLD}_all_candidates'
MODEL_SAVING_PATH = f'models/model-{MODEL_NAME}/weights'
HISTORY_JSON_PATH = f'history/training_history_{MODEL_NAME}.json'  # Path to save the history JSON
TRAIN_SPLIT = 0.8
SEED = 119
...
def prepare_data(
    ...
    pairs = pd.merge(pairs, collection, on='id')
    pairs.dropna(subset=['query', 'product_embedding'], inplace=True)
    collection['product_embedding'] = collection['product_embedding'].apply(np.array)
    ...
    pairs = pairs[pairs['cosine_similarity'] >= SIMILARITY_THRESHOLD]
    total_samples: int = len(pairs)
    
    unique_embeddings = pairs.groupby('id')['product_embedding'].first().values
    if ALL_CANDIDATES:
        unique_embeddings = collection['product_embedding'].values
