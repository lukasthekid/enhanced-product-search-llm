...
def get_top_n_index(index, query_embedding, collection, k=10):
    distances, indices = index.search(query_embedding, k)
    top_results = collection.iloc[indices[0]].copy()
    top_results['score'] = distances[0]
    top_results = top_results.sort_values(by='score', ascending=True).reset_index(drop=True)
    return top_results.head(k)
